...
using RhyoliteERP.Web.Models.Account;
using RhyoliteERP.Web.Views.Shared.Components.TenantChange;
using RhyoliteERP.Web.Models.Dto;
using RhyoliteERP.Services.Shared.Messaging;

namespace RhyoliteERP.Web.Controllers
{
    public class AccountController : RhyoliteERPControllerBase
    {
        ...
        public ActionResult SignIn(string userNameOrEmailAddress = "", string returnUrl = "", string successMessage = "")
        {
        ...
        public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            var loginResult = await GetLoginResultAsync(loginModel.UsernameOrEmailAddress, loginModel.Password, loginModel.AccountName);
        ...
        {
            return RegisterSignupView(new RegisterViewModel());
        }

        private ActionResult RegisterSignupView(RegisterViewModel model)
        {
        ...
        public async Task<ActionResult> SendInvitation([FromBody] InvitationDto invitation)
        {

            string invitationMessage = $"***New Business Registration***\nBusiness Name: {invitation.BusinessName}\nPhoneNo: {invitation.PrimaryPhone}\nEmail: {invitation.PrimaryEmail}\nCity: {invitation.City}\nLocation: {invitation.LocationOrAddress}";

            await _messagingAppService.NotifyInvitation(new DomainServices.School.Messaging.Dto.StartCampaign { Message = invitationMessage , MessagingChannel = "telegram", ModuleSource = 0, TenantId = 0, Subject = "New Business Registration",   });

            return Json(new { code = 200 , message = "Registration successful." });
        }
        ...
        {
            return Url.Action("Modules", "Manage");
        }
