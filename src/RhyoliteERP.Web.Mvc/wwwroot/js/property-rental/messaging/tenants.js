    ...
    methods: {
        ...
        getTenants() {
            ...
            axios.get('/Rentals/GetLeaseTenants').then(response => {
                ...
                response.data.forEach((v) => {
                    ...
                    v.tenantName = `${v.firstName} ${v.lastName} - ${v.primaryPhoneNo}`
                });

                this.leaseTenantList = response.data

                $("#leaseTenantId").ejDropDownList({
                    dataSource: response.data,
                    watermarkText: "Search Tenants",
                    width: "100%",
                    fields: { id: "id", text: "tenantName", value: "id" },
                    enableFilterSearch: true,
                    change: (args) => {

                        this.leaseTenantId = args.value;
                    }
        ...
        sendMessage() {
            ...
            if (this.isDirectMessage && !this.parentId) {
                ...
                this.toastHandler('warning', 'Select Tenant');
                return;
            ...
            this.payload.recipientType = 'lease-tenant';
            this.payload.recipientCount = this.leaseTenantList.length;
