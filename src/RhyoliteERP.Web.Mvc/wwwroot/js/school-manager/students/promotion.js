...
var vue = new Vue({
    ...
    data: {
        ...
        promotedFromClassId: '',
        promotedToClassId: '',
        datePromoted: moment().format("YYYY-MM-DD"),
        schClasses: [],
    ...
    methods: {
       ...
       renderTable(responseData) {
        ...
        promoteStudents() {
            ...
            if (this.promotedFromClassId == '' && this.promotedToClassId == '') {

                this.toastHandler('warning', 'Source and Destination Classes are required for Promotion..!');

                return;
            }

            if (this.promotedFromClassId != '' && this.promotedToClassId != '' && this.promotedFromClassId == this.promotedToClassId) {

                this.toastHandler('warning', 'Source and Destination Classes cannot the same..!');
            ...
            this.studentsPromotionData.forEach( (v) => {
                ...
                v.classId = this.promotedToClassId;
               
            });

            if (this.studentsPromotionData.length != 0) {

                this.payload.promotedFrom = this.promotedFromClassId;
                this.payload.promotedTo = this.promotedToClassId;
                this.payload.datePromoted = this.datePromoted;
                this.payload.students = this.studentsPromotionData;


                axios.post('/Students/Promote', this.payload).then(response => {

                    this.toastHandler('', 'Students Promotion Successful..!');
                    this.isLoading = false;
                    this.getStudentsByClass();

                }).catch(err => {

                    this.toastHandler('error', 'Unable to promote students.');
                    this.isLoading = false;
